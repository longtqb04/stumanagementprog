...
public:
    static Student* createStudent(const string& name, const string& dob, const string& school, const string& course, const int& courseType) {
        if (courseType == 1) { // if University
            return new UniStudent(name, dob, school, course, courseType);
        } else if (courseType == 2) { // if College
            return new CollegeStudent(name, dob, school, course, courseType);
        } else {
            return nullptr;
        }
    }
...
void addStudent(Student**& students, int& numStudents) {
    ...
    cin >> courseType;
    Student* newStudent = StudentFactory::createStudent(name, dob, schoolName, courseName, courseType);
